...
let htmlstream='';
    ...
    if (body.uid == '' || body.pw1 == '') {
       ...
       db.query('INSERT INTO u0_users (uid, pass, name, phone,address,point) VALUES (?, ?, ?, ?, ?, ?)', [body.uid, body.pw1, body.uname, body.uphone, body.uaddress, 500], (error, results, fields) => {
          if (error) {
            htmlstream = fs.readFileSync(__dirname + '/../../views/common/alert.ejs','utf8');
            res.status(600).end(ejs.render(htmlstream, { 'title': '알리미',
                               'warn_title':'회원가입 오류',
...
const HandleLogin = (req, res) => {
  ...
  let htmlstream = '';
      ...
      if (body.uid == '' || body.pass == '') {
       ...
       sql_str = "SELECT uid, pass, name from u0_users where uid ='"+ body.uid +"' and pass='" + body.pass + "';";
       ...
       db.query(sql_str, (error, results, fields) => {
         if (error) { res.status(601).end("Login Fail as No id in DB!"); }
            ...
            if (results.length <= 0) {  // select 조회결과가 없는 경우 (즉, 등록계정이 없는 경우)
                  htmlstream = fs.readFileSync(__dirname + '/../../views/common/alert.ejs','utf8');
                  res.status(601).end(ejs.render(htmlstream, { 'title': '알리미',
                                     'warn_title':'로그인 오류',
